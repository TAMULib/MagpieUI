...
```shell
docker image build -t magpieui .
docker run -it magpieui
```

<sub>_* Note: `-t magpieui` and `-it magpieui` may be changed to another tag name as desired, such as `-t developing_on_this` and `-it developing_on_this`._</sub><br>
